...
impl Tensor {
    ...
    ///
    /// # Errors
    /// Returns an error if the tensor is on GPU or if type doesn't match.
    pub fn as_slice<T: Pod>(&self) -> Result<&[T]> {
        match &self.data {
            TensorData::Cpu(bytes) => {
                if std::mem::size_of::<T>() * self.len() != bytes.len() {
                    return Err(RuntimeError::TensorError(
                        "Type size mismatch".to_string(),
                    ));
                }
                Ok(bytemuck::cast_slice(bytes))
            }
            TensorData::Gpu(_) => Err(RuntimeError::TensorError(
                "Cannot get slice of GPU tensor".to_string(),
            )),
        }
    }
    ...
    ///
    /// # Errors
    /// Returns an error if type doesn't match or tensor is on GPU.
    ...
    pub(crate) fn raw_data(&self) -> Result<&[u8]> {
        match &self.data {
            TensorData::Cpu(bytes) => Ok(bytes),
            TensorData::Gpu(_) => Err(RuntimeError::TensorError(
                "Cannot get raw data of GPU tensor".to_string(),
            )),
        }
    }
...
pub(crate) enum TensorData {
    ...
    /// Reference to GPU buffer (device memory).
    Gpu(GpuBufferRef),
}

/// Reference to a GPU buffer.
#[derive(Debug, Clone)]
pub(crate) struct GpuBufferRef {
    /// Index into buffer manager.
