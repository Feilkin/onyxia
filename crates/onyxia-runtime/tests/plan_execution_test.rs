...
/// Helper function to create a simple Gather graph programmatically.
///
/// Graph structure:
/// - Inputs: data:[f32;4×3], indices:[i64;2]
/// - Operation: Gather(data, indices) -> output
/// - Output: output:[f32;2×3]
...
///
/// This test verifies:
/// 1. Graph construction for Gather operation
/// 2. Compilation to ExecutionPlan
/// 3. GPU execution with I64 indices
/// 4. Correct indexing and data retrieval
#[pollster::test]
#[ignore] // Requires GPU
async fn test_gather_e2e() {
    ...
    assert_eq!(
        plan.operations.len(),
        1,
        "Should have exactly 1 operation (Gather)"
    );
    assert_eq!(plan.shaders.len(), 1, "Should have exactly 1 shader");
    assert_eq!(plan.inputs.len(), 2, "Should have 2 inputs");
    assert_eq!(plan.outputs.len(), 1, "Should have 1 output");
    ...
    assert_eq!(output.len(), 6, "Output should have 6 elements (2×3)");
...
///
/// This test simulates a more realistic use case like Gemma's embedding layer:
...
async fn test_gather_embedding_e2e() {
    ...
    graph.add_tensor(TensorInfo {
        ...
        dtype: DataType::F32,
        shape: TensorShape::Static(vec![8, 4]), // vocab_size=8, hidden_dim=4
        kind: TensorKind::Input,
        initializer: None,
    });
    ...
    assert_eq!(
        embeddings.len(),
        24,
        "Output should have 24 elements (2×3×4)"
    );

    // Expected embeddings:
    // Batch 0, Seq 0 (token 0): [0.1, 0.2, 0.3, 0.4]
    // Batch 0, Seq 1 (token 2): [2.1, 2.2, 2.3, 2.4]
    // Batch 0, Seq 2 (token 4): [4.1, 4.2, 4.3, 4.4]
    // Batch 1, Seq 0 (token 1): [1.1, 1.2, 1.3, 1.4]
    // Batch 1, Seq 1 (token 3): [3.1, 3.2, 3.3, 3.4]
    // Batch 1, Seq 2 (token 5): [5.1, 5.2, 5.3, 5.4]
    let expected = vec![
