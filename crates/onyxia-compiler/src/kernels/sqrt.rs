//! SqrtKernel implementation for square root function.
...
mod tests {
    ...
    fn test_sqrt_infer_shapes() {
        ...
        let result = kernel.infer_output_shapes(&ctx).unwrap();
    ...
    fn test_sqrt_constant_fold() {
        ...
        let result = kernel.try_fold(&ctx).unwrap();
        assert_eq!(result.len(), 1);
    ...
    fn test_sqrt_kernel_plan() {
        ...
        let steps = SqrtKernel.plan(&mut ctx).expect("Planning should succeed");
