//! SinKernel implementation for sine activation function.
...
/// Kernel for Sin activation (ONNX Sin operator).
///
/// Sin(x) = sin(x) - element-wise sine function.
pub struct SinKernel;

impl OpKernel for SinKernel {
    fn name(&self) -> &str {
        "Sin"
    }
...
mod tests {
    ...
    fn test_sin_kernel_empty_input() {
        ...
        let result = kernel.infer_output_shapes(&ctx);
