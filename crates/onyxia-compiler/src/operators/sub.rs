//! SubOperator implementation for elementwise subtraction.
...
/// Operator for elementwise subtraction (ONNX Sub operator).
///
...
impl OpOperator for SubOperator {
    fn name(&self) -> &str {
        "Sub"
    }
...
mod tests {
    ...
    fn test_sub_kernel_plan() {
        ...
        let steps = SubOperator.plan(&mut ctx).expect("Planning should succeed");
    ...
    fn test_sub_kernel_shader_deduplication() {
        ...
        let _steps1 = SubOperator
            .plan(&mut ctx)
            .expect("First planning should succeed");
        let steps2 = SubOperator
            .plan(&mut ctx)
