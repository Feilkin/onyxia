//! PowOperator implementation for elementwise power operation.
...
/// Operator for elementwise power operation (ONNX Pow operator).
///
...
impl OpOperator for PowOperator {
    fn name(&self) -> &str {
        "Pow"
    }
...
mod tests {
    ...
    fn test_pow_kernel_name() {
        let operator = PowOperator;
        assert_eq!(operator.name(), "Pow");
    }
