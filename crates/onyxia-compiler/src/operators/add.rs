//! AddOperator implementation for elementwise addition.
...
mod tests {
    ...
    fn test_add_operator_plan() {
        ...
        let steps = AddOperator.plan(&mut ctx).expect("Planning should succeed");
    ...
    #[test]
    fn test_add_operator_larger_tensor() {
        let mut graph = Graph::new();
    ...
    #[test]
    fn test_add_operator_non_divisible_size() {
        let mut graph = Graph::new();
    ...
    #[test]
    fn test_add_operator_shader_deduplication() {
        let graph = create_add_test_graph();
        let mut node = Node::new("Add");
        node.inputs = vec!["a".to_string(), "b".to_string()];
        node.outputs = vec!["c".to_string()];
        ...
        let _steps1 = AddOperator
            .plan(&mut ctx)
            .expect("First planning should succeed");
        let steps2 = AddOperator
            .plan(&mut ctx)
