//! CosOperator implementation for cosine activation function.
...
/// Operator for Cos activation (ONNX Cos operator).
///
    ...
    fn try_fold(&self, ctx: &InferenceContext<'_>) -> Result<Vec<Option<TensorValue>>> {
        ...
        match input {
            TensorValue::F32(vals) => {
                let result: Vec<f32> = vals.iter().map(|x| x.cos()).collect();
...
mod tests {
    ...
    fn test_cos_kernel_plan() {
        ...
        let steps = CosOperator.plan(&mut ctx).expect("Planning should succeed");
