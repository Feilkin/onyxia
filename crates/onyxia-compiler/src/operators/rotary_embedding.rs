//! RotaryEmbeddingOperator implementation for Rotary Position Embedding (RoPE).
...
/// Operator for Rotary Position Embedding (Microsoft contrib op "RotaryEmbedding").
///
...
pub struct RotaryEmbeddingOperator;

impl OpOperator for RotaryEmbeddingOperator {
    fn name(&self) -> &str {
        "RotaryEmbedding"
    }
...
mod tests {
    ...
    fn test_rotary_embedding_kernel_shape_inference() {
        let operator = RotaryEmbeddingOperator;
    ...
    fn test_rotary_embedding_kernel_plan() {
        ...
        let steps = RotaryEmbeddingOperator
            .plan(&mut ctx)
    ...
    fn test_rotary_embedding_kernel_attributes() {
        ...
        let result = RotaryEmbeddingOperator.plan(&mut ctx);
        assert!(
            result.is_ok(),
