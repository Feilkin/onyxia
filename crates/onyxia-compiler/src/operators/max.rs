//! MaxOperator implementation for elementwise maximum.
...
/// Operator for elementwise maximum (ONNX Max operator).
///
...
mod tests {
    ...
    fn test_max_kernel_name() {
        let operator = MaxOperator;
        assert_eq!(operator.name(), "Max");
    }
    ...
    fn test_max_kernel_plan() {
        ...
        let steps = MaxOperator.plan(&mut ctx).expect("Planning should succeed");
    ...
    fn test_max_try_fold() {
        ...
        let ctx = InferenceContext::new(&node, &graph, input_shapes, input_values);
        let folded = operator.try_fold(&ctx).expect("Should fold");
