//! ConstantOperator implementation for ONNX Constant nodes.
...
/// Operator for ONNX Constant operator.
///
...
mod tests {
    ...
    fn test_constant_kernel_empty_steps() {
        ...
        let operator = ConstantOperator;
        let steps = operator.plan(&mut ctx).expect("plan should succeed");

        // Verify that no GPU steps are emitted
        assert_eq!(
            steps.len(),
            0,
            "ConstantOperator should emit zero steps (data is in initializer)"
        );
    }
