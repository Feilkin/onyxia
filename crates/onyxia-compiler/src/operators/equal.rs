//! EqualOperator implementation for elementwise equality comparison.
...
/// Kernel for elementwise equality comparison (ONNX Equal operator).
///
...
/// ## Output Type
///
/// The ONNX spec defines Equal's output as boolean (DataType::Bool). Since WGSL
...
impl OpOperator for EqualOperator {
    fn name(&self) -> &str {
        "Equal"
    }
    ...
    fn plan(&self, ctx: &mut PlanContext<'_>) -> Result<Vec<Step>> {
        ...
        let shader_index = ctx.compile_shader(
            "equal",
            include_str!("../../shaders/elementwise/equal.wgsl"),
            shader_defs,
        )?;
...
mod tests {
    ...
    fn create_equal_test_graph() -> Graph {
    ...
    #[test]
    fn test_equal_kernel_basic() {
        let operator = EqualOperator;
        assert_eq!(operator.name(), "Equal");
    }

    #[test]
    fn test_equal_kernel_plan() {
        let operator = EqualOperator;
        let graph = create_equal_test_graph();
        ...
        assert_eq!(shaders[0].label, "equal");
        assert_eq!(shaders[0].entry_point, "main");
    }

    #[test]
    fn test_equal_kernel_infer_shapes() {
        let operator = EqualOperator;
        let graph = create_equal_test_graph();
