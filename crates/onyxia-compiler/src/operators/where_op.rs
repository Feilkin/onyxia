//! WhereOperator implementation for conditional element selection.
...
/// Operator for conditional element selection (ONNX Where operator).
///
...
impl OpOperator for WhereOperator {
    fn name(&self) -> &str {
        "Where"
    }
...
mod tests {
    ...
    fn test_where_kernel_plan() {
        ...
        let steps = WhereOperator.plan(&mut ctx).expect("Planning should succeed");
