//! DivOperator implementation for elementwise division.
...
/// Operator for elementwise division (ONNX Div operator).
///
...
impl OpOperator for DivOperator {
    fn name(&self) -> &str {
        "Div"
    }
    ...
    fn plan(&self, ctx: &mut PlanContext<'_>) -> Result<Vec<Step>> {
        ...
        let shader_index = ctx.compile_shader(
            "div",
            include_str!("../../shaders/elementwise/div.wgsl"),
            shader_defs,
        )?;
...
mod tests {
    ...
    fn create_div_test_graph() -> Graph {
    ...
    #[test]
    fn test_div_kernel_plan() {
        let graph = create_div_test_graph();
        ...
        let steps = DivOperator.plan(&mut ctx).expect("Planning should succeed");
        ...
        assert_eq!(shaders[0].label, "div");
        assert_eq!(shaders[0].entry_point, "main");
    }

    #[test]
    fn test_div_kernel_broadcast() {
        let mut graph = Graph::new();
