// ReduceMean: Compute arithmetic mean along specified axes
//
// Algorithm:
...
// - Final thread divides by reduction count to get mean
...
fn main(
    ...
    // Thread 0 writes the final mean (sum / count)
    if (thread_idx == 0u) {
        let total_sum = partial_sums[0];
        let mean = total_sum / f32(params.reduction_size);
