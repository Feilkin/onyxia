...
//!
//! Computes the sum of input tensor elements along specified axes.
...
/// ReduceSum operator.
///
/// Computes the sum of input tensor elements across specified dimensions.
///
/// **ONNX Specification:**
/// - Opset: 13
/// - Inputs:
...
///
/// **Implementation:**
/// - Uses parallel reduction via workgroup shared memory
/// - Each workgroup handles one output element
/// - Threads cooperatively sum elements via tree reduction
pub struct ReduceSumOp;

impl Operator for ReduceSumOp {
    fn name(&self) -> &str {
        "ReduceSum"
    }
    ...
    fn create_dispatch(&self, ctx: &mut CompileCtx) -> Result<Box<dyn OpDispatch>> {
        ...
        let axes: Vec<i64> = if ctx.input_count() > 1 {
            ...
            if let Some(value) = ctx.input_value(1)? {
                ...
                return Err(Error::Unsupported(
                    "ReduceSum with runtime axes is not yet supported".into(),
                ));
            }
        ...
        let module = ctx.compile_shader("ReduceSum", REDUCE_SUM_SHADER, &shader_defs)?;

        Ok(Box::new(ReduceSumDispatch {
            module,
            label: "ReduceSum".to_string(),
            axes,
            keepdims,
...
/// Runtime dispatch for ReduceSum operation.
struct ReduceSumDispatch {
...
mod tests {
    ...
    #[test]
    fn test_parse_axes_i64() {
        // Simulate i64 axes tensor: [0, 2]
        let axes_i64 = vec![0i64, 2];
        let bytes: Vec<u8> = axes_i64.iter().flat_map(|&x| x.to_le_bytes()).collect();

        // Axes parsing would happen in dispatch, but we can test the byte layout
        assert_eq!(bytes.len(), 16); // 2 i64 values = 16 bytes

        // Verify first axis (0)
        let first = i64::from_le_bytes(bytes[0..8].try_into().unwrap());
        assert_eq!(first, 0);

        // Verify second axis (2)
        let second = i64::from_le_bytes(bytes[8..16].try_into().unwrap());
        assert_eq!(second, 2);
    }
