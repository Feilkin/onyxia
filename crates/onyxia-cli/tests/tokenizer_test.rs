...
fn test_tokenizer_encode() -> Result<()> {
    ...
    assert!(
        token_ids.len() >= 2,
        "Expected at least 2 tokens for 'Hello, world!'"
    );
    ...
    assert_eq!(
        token_ids_with_bos.len(),
        token_ids.len() + 1,
        "BOS should add one token"
    );
...
fn test_tokenizer_decode() -> Result<()> {
    ...
    assert!(
        decoded.to_lowercase().contains("world"),
...
fn test_chat_template_load() -> Result<()> {
    ...
    assert!(
        ...
        "Prompt should contain user turn marker"
    );
    assert!(
        prompt.contains("Hello!"),
        "Prompt should contain the message content"
    );
    assert!(
        prompt.contains("<end_of_turn>"),
        "Prompt should contain end turn marker"
    );
    assert!(
        prompt.contains("<start_of_turn>model"),
        "Prompt should contain model turn marker for generation"
    );
...
fn test_chat_template_multi_turn() -> Result<()> {
    ...
    assert!(
        prompt.contains("What about 3+3?"),
...
fn test_encode_chat_template_output() -> Result<()> {
    ...
    assert!(
        token_ids.len() > 5,
        "Expected more than 5 tokens for formatted prompt"
    );
