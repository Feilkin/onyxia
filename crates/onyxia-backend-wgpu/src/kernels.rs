...
/// the storage buffers: 128×`tn` output tile (`tn` = 64 or 128) per
/// 256-thread workgroup, 8 subgroups (32 wide) as 4 row-groups × 2
/// column-groups, each owning 32 × `tn/2` as 2×(`tn`/32) `16×16×16`
/// MMAs, K stepped by 16 with no workgroup memory and no barriers in
/// the loop. `a` is row-major `[M,K]` (`coopLoadT`, stride K); `b` is
/// row-major `[K,N]` (`coopLoadT`, stride N) or `[N,K]` (`coopLoad`,
/// stride K). Requires `K % 16 == 0`, `N % 16 == 0` and `a` padded to
/// a multiple of 16 rows (the caller copies); the f32 accumulators drain
/// through a 16 KB workgroup slot two tiles per subgroup per round for
/// the bounds-checked, type-converted store. Same immediates and grid
/// convention as [`matmul_tiled_rb`] (`z` = K slice; batch is 1). The
/// wide tile halves the A re-reads per output column; the session
/// picks it for wide N.
pub fn matmul_coop_f16(trans_b: bool, out: MatElem, tn: usize) -> String {
    assert!(tn == 64 || tn == 128, "coop tile width is 64 or 128");
    let o = out.wgsl();
    let jt = tn / 32; // column tiles per subgroup
    let b_load = |j: usize| {
        if trans_b {
            format!("coopLoad<coop_mat16x16<f16, B>>(&b[(nb + {j}u * 16u) * p.k + k0], p.k)")
        } else {
            format!("coopLoadT<coop_mat16x16<f16, B>>(&b[k0 * p.n + nb + {j}u * 16u], p.n)")
        }
    };
    let decl: String = (0..2)
        .flat_map(|i| (0..jt).map(move |j| format!("    var c{i}{j} = coop_mat16x16<f32, C>();\n")))
        .collect();
    let loads: String = (0..jt)
        .map(|j| format!("        let b{j} = {};\n", b_load(j)))
        .collect();
    let mmas: String = (0..2)
        .flat_map(|i| {
            (0..jt)
                .map(move |j| format!("        c{i}{j} = coopMultiplyAdd(a{i}, b{j}, c{i}{j});\n"))
        })
        .collect();
    // Drain rounds: tiles (i, j) and (i, j+1) per round, into
    // slot[sg][2][16×16].
    let drain: String = (0..2)
        .flat_map(|i| {
            (0..jt).step_by(2).map(move |j| {
                format!(
                    "    coopStoreT(c{i}{j}, &Cs[sg * 512u], 16u);
    coopStoreT(c{i}{j1}, &Cs[sg * 512u + 256u], 16u);
    workgroupBarrier();
    for (var e = lane; e < 512u; e += 32u) {{
        let tile = e / 256u;
        let idx = e % 256u;
        let mm = ma + {i}u * 16u + idx / 16u;
        let nn = nb + ({j}u + tile) * 16u + idx % 16u;
        if (mm < p.m && nn < p.n) {{
            out[out_base + mm * p.n + nn] = {o}(Cs[sg * 512u + e]);
        }}
    }}
    workgroupBarrier();\n",
                    j1 = j + 1
...
fn main(@builtin(workgroup_id) wg: vec3<u32>,
    ...
    let k_lo = wg.z * p.chunk;
    let k_hi = min(k_lo + p.chunk, p.k);
    // Subgroup (qm, qn) owns rows m0 + qm*32 .. +32, cols n0 + qn*{hn} .. +{hn}.
    let qm = sg / 2u;
    let qn = sg % 2u;
    let ma = m0 + qm * 32u;
    let nb = n0 + qn * {hn}u;
{decl}    for (var k0 = k_lo; k0 < k_hi; k0 += 16u) {{
