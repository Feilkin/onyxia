//! SliceKernel implementation for tensor slicing operations.
...
/// Kernel for Slice operation (ONNX Slice operator).
///
...
mod tests {
    ...
    fn test_slice_kernel_plan() {
        ...
        let steps = SliceKernel.plan(&mut ctx).expect("Planning should succeed");
    ...
    fn test_normalize_index() {
        ...
        // Positive indices
        assert_eq!(kernel.normalize_index(0, 10), 0);
        assert_eq!(kernel.normalize_index(5, 10), 5);
        assert_eq!(kernel.normalize_index(10, 10), 10);
        assert_eq!(kernel.normalize_index(15, 10), 10); // Clamped

        // Negative indices
        assert_eq!(kernel.normalize_index(-1, 10), 9);
        assert_eq!(kernel.normalize_index(-5, 10), 5);
        assert_eq!(kernel.normalize_index(-10, 10), 0);
        assert_eq!(kernel.normalize_index(-15, 10), 0); // Clamped
    }
}
