//! ReduceMeanKernel implementation for mean reduction along axes.
...
/// Kernel for mean-reducing a tensor along specified axes (ONNX ReduceMean operator).
///
/// Reduces the input tensor by computing the mean along the specified axes.
/// Supports `keepdims` attribute to retain reduced dimensions as size 1.
pub struct ReduceMeanKernel;

impl OpKernel for ReduceMeanKernel {
    fn name(&self) -> &str {
        "ReduceMean"
    }
    ...
    fn infer_output_shapes(&self, ctx: &InferenceContext<'_>) -> Result<Vec<TensorShape>> {
        if ctx.input_shapes.is_empty() {
            return Err(crate::error::CodegenError::InvalidShape(
                "ReduceMean requires at least one input".to_string(),
            ));
        }
        ...
        let input_dims = match &ctx.input_shapes[0] {
            ...
            TensorShape::Absent => {
                return Err(crate::error::CodegenError::InvalidShape(
                    "ReduceMean input is absent".to_string(),
                ));
            }
    ...
    fn plan(&self, ctx: &mut PlanContext<'_>) -> Result<Vec<Step>> {
        ...
        if axes.len() != 1 {
            return Err(crate::error::CodegenError::UnsupportedOp(format!(
                "ReduceMean currently only supports single-axis reduction, got {} axes",
                axes.len()
            )));
        }
        ...
        // Each workgroup computes one output element by computing mean along the reduction axis
        ...
        let shader_index = ctx.compile_shader(
            "reducemean",
            include_str!("../../shaders/reduction/reducemean.wgsl"),
            shader_defs,
        )?;
...
mod tests {
    ...
    #[test]
    fn test_reducemean_kernel_plan() {
        ...
        assert_eq!(shaders[0].label, "reducemean");
    }

    #[test]
    fn test_reducemean_kernel_keepdims() {
        let mut node = Node::new("ReduceMean");
        node.attributes
            .insert("axes".to_string(), AttributeValue::Ints(vec![1i64]));
        node.attributes
        ...
        let output_shapes = ReduceMeanKernel
            .infer_output_shapes(&{
    ...
    #[test]
    fn test_reducemean_kernel_no_keepdims() {
        let mut node = Node::new("ReduceMean");
        node.attributes
            .insert("axes".to_string(), AttributeValue::Ints(vec![1i64]));
        node.attributes
