...
//!
//! ```
//! use onyxia_core::symbolic_expr::{SymbolicExpr, parse_expr, evaluate_expr};
...
pub enum SymbolicExpr {
    ...
    Variable(String),
    /// Binary operation
    BinOp(Box<SymbolicExpr>, BinOpKind, Box<SymbolicExpr>),
}
...
/// ```
pub fn parse_expr(input: &str) -> Result<SymbolicExpr, String> {
...
pub fn evaluate_expr(expr: &SymbolicExpr, vars: &HashMap<String, usize>) -> Result<usize, String> {
    match expr {
        SymbolicExpr::Literal(n) => {
            if *n < 0 {
                return Err(format!("Dimension cannot be negative: {}", n));
            }
            Ok(*n as usize)
        }
        SymbolicExpr::Variable(name) => vars
            .get(name)
            .copied()
            .ok_or_else(|| format!("Variable '{}' not found in dynamic_dimensions", name)),
        SymbolicExpr::BinOp(left, op, right) => {
...
impl Parser {
    ...
    fn parse_expr(&mut self) -> Result<SymbolicExpr, String> {
        self.parse_add_sub()
    }

    fn parse_add_sub(&mut self) -> Result<SymbolicExpr, String> {
        ...
        while let Some(token) = self.current() {
            ...
            left = SymbolicExpr::BinOp(Box::new(left), op, Box::new(right));
        }

        Ok(left)
    }

    fn parse_mul_div_mod(&mut self) -> Result<SymbolicExpr, String> {
    ...
    fn parse_factor(&mut self) -> Result<SymbolicExpr, String> {
        match self.current() {
            Some(Token::Number(n)) => {
                let n = *n;
                self.advance();
                Ok(SymbolicExpr::Literal(n))
            }
            Some(Token::Ident(name)) => {
                let name = name.clone();
                self.advance();
                Ok(SymbolicExpr::Variable(name))
            }
            Some(Token::LParen) => {
                self.advance();
...
mod tests {
    ...
    fn test_parse_simple_identifier() {
        let expr = parse_expr("batch_size").unwrap();
        assert_eq!(expr, SymbolicExpr::Variable("batch_size".to_string()));
    }

    #[test]
    fn test_parse_number() {
        let expr = parse_expr("42").unwrap();
        assert_eq!(expr, SymbolicExpr::Literal(42));
    }

    #[test]
    fn test_parse_addition() {
        let expr = parse_expr("a + b").unwrap();
        assert!(matches!(expr, SymbolicExpr::BinOp(_, BinOpKind::Add, _)));
    }

    #[test]
    fn test_parse_multiplication() {
        let expr = parse_expr("seq * heads").unwrap();
        assert!(matches!(expr, SymbolicExpr::BinOp(_, BinOpKind::Mul, _)));
    }
    ...
    fn test_parse_complex_expression() {
        ...
        assert!(matches!(expr, SymbolicExpr::BinOp(_, BinOpKind::Div, _)));
    }
    ...
    fn test_parse_operator_precedence() {
        ...
        match expr {
            SymbolicExpr::BinOp(_, BinOpKind::Add, right) => {
                assert!(matches!(*right, SymbolicExpr::BinOp(_, BinOpKind::Mul, _)));
            }
