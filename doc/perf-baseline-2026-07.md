...
> **Status update 4 (2026-07-04):** vec4 matvec loads. `[N,K]`
> (`trans_b`) now loads vec4 with **4 rows per workgroup** (one 2.5 KB
> row per 256-thread workgroup was latency-bound): lm_head 3.56 →
> 2.07 ms (302 GiB/s). `[K,N]` keeps its 64-column tile as 16 vec4 ×
> 16 K-lanes (a wider 256-column tile cost occupancy on the small
> projections — measured, reverted). Scalar kernels remain the fallback
> when K/N aren't 4-aligned. End to end: **decode 7.68 ms/tok
> (130 tok/s)**, prefill 194 ms.
>
> **Measurement hygiene:** everything here runs sequentially — criterion
> executes cases one at a time in one process, and `onyxia bench` is a
> single process (GPU *tests* under nextest do run in parallel, but
> they're pass/fail, never timed). The thing that *does* muddy numbers
> is GPU clock state: the card idles at 225 MHz and re-boosts per run,
> which shows up as ±4–6 % between identical back-to-back runs (with
> confident-looking p-values). Treat cross-run deltas under ~5 % as
> noise, or lock clocks (`nvidia-smi -lgc`) for fine comparisons.
>
> **Status update 3 (2026-07-04):** CPU dispatch overhead pass, guided by
> `perf` (no wgpu validation layers were involved — release builds run
> with `InstanceFlags::empty()`): bind groups are now cached by buffer
> identity (Weak-pinned pointer keys), WGSL source is only generated on
> pipeline-cache miss, all dispatches in a batch share one compute pass
> when not profiling, liveness uses precomputed per-step death lists, and
> `Tensor::to_f32` no longer round-trips the 262k-logit vector through
> f64. Measured: **decode 9.35 ms/tok (107 tok/s)** — now ~85 % GPU-bound
> (GPU busy ≈ 7.9 ms) — prefill 266 ms, VRAM unchanged. NVIDIA driver
> CPU share fell from 44 % to 24 % of cycles. Note when reading
> `--profile` runs: profiling forces one pass per dispatch, which
> inflates wall time; take throughput from unprofiled runs. Next GPU-side
> levers: vec4 loads in the matvec kernels, then it's kernel math all the
> way down.
>
